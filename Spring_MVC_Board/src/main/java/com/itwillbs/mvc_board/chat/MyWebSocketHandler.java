...
import com.itwillbs.mvc_board.vo.ChatMessageVO;
...
public class MyWebSocketHandler extends TextWebSocketHandler {
	...
	//    (멀티스레드 환경에서 Hashmap은 안전성을 보장하지않는다)
	//    (여기저기서 접속하니까 Hashmap은 위험)
...
//		System.out.println("연결된 세션 ID : " + session.getId());
		...
		System.out.println("접속 클라이언트 목록 : " + session);
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		System.out.println(session.getId() + "로부터 전달받은 메세지 : " + message.getPayload());
...
//		System.out.println("메시지 : " + jo.getString("message"));
		...
		// ----------------------------------------------------------------------------------------------------
		// 메세지 타입 판별(ChatMessageVO 객체의 type 멤버변수값 활용)
		if(chatMessage.getType().equals(ChatMessageVO.TYPE_ENTER)) { // 입장
//			System.out.println(session.getId() + " : 입장!");
		...
		} else if(chatMessage.getType().equals(ChatMessageVO.TYPE_LEAVE)) { // 퇴장
...
//		session.sendMessage(new TextMessage("서버에서 메세지 수신 완료!")); // CharSequence 타입 = String
...
//		session.sendMessage(new TextMessage(gson.toJson(chatMessage)));
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		System.out.println("웹소켓 연결이 해제됨!(afterConnectionClosed)");
	}
