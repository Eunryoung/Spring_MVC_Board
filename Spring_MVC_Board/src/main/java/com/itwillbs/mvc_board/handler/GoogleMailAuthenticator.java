...
// 자바 메일 기능 사용 시 메일 서버(e.g. Gmail) 인증을 위한 정보 관리할 클래스 정의
...
public class GoogleMailAuthenticator extends Authenticator {
	...
	public GoogleMailAuthenticator() {
		 ...
		 *   => Gmail 기준 2단계 인증 사용 시 : Gmail 계정명, 2단계 인증 우회 앱 비밀번호
		 *   => 구글 계정 설정 - 보안 - 2단계 인증 - 앱 비밀번호 설정 필요
		 *      (설정 시 나타나는 16자리 앱 비밀번호를 계정 패스워드 대신 사용(로그인 시에는 사용 불가))
		 */
		passwordAuthentication = new PasswordAuthentication("eri12nov", "abdsqkgvwnerywef");
