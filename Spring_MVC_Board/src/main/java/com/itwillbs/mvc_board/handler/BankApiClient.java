...
//		headers.add("Authorization", "Bearer " + map.get("access_token"));
		...
		jo.put("cntr_account_type", "N"); // 약정 계좌/계정 구분("N" : 계좌, "C" : 계정 => 개념 잘 모르니까 우리는 일단 N으로 고정)
		jo.put("cntr_account_num", cntr_account_num); // 약정 계좌/계정 번호(핀테크 서비스 기관의 계좌번호) => 아이티윌 계좌번호(강사님 샘플 번호)
		jo.put("dps_print_content", map.get("id")); // 입금계좌인자내역(결제 요청 사용자 아이디 활용) (돈 받는 사람한테 찍히는 내역 => 현재는 아이티윌 계좌에 찍힐 내역, 우리는 사용자 id 출력하기로ㅇㅇ)
		
		// ------------ 요청 고객(출금 대상) 정보 ------------
		jo.put("fintech_use_num", map.get("fintech_use_num")); // 출금계좌 핀테크이용번호
		jo.put("wd_print_content", "아이티윌_결제"); // 출금계좌인자내역(현재 센터 인증이므로 기관(아이티윌)에서 설정하는 내용 아무거나 적자, 우리는 메시지 자체지정)
		jo.put("tran_amt", map.get("tran_amt")); // 거래금액
		jo.put("tran_dtime", bankValueGenerator.getTranDTime()); // 요청일시(자동생성)
		
		jo.put("req_client_name", map.get("req_client_name")); // 요청고객 성명(출금계좌 예금주명) => 현 실습에서는 내 이름
		...
		// "req_client_name":"임은령",
		// "transfer_purpose":"ST",
		// "tran_dtime":"20240124121214",
		// "bank_tran_id":"M202113854UNUVPMRRUL",
		// "req_client_num":"ADMIN",
		// "fintech_use_num":"120211385488932395653894",
		// "wd_print_content":"아이티윌_결제",
		// "recv_client_account_num":"",
		// "dps_print_content":"admin",
		// "req_client_fintech_use_num":"120211385488932395653894",
		// "recv_client_name":"",
		// "cntr_account_type":"N",
		// "tran_amt":"15000",
		 ...
		 * 수취인 성명 : 핀테크 이용기관의 계좌 예금주명 입력(최종 수취인과 다를 수 있음!) => 현 실습에서는 강사님 성함!!!
		 */
	}
...
//		String url = "https://testapi.openbanking.or.kr/v2.0/transfer/deposit/fin_num";
		...
		joReq.put("tran_no", "1"); // 거래순번(현재 단건만 가능하므로 무조건 1)
		joReq.put("bank_tran_id", bankValueGenerator.getBankTranId()); // 거래고유번호(자동생성)
		joReq.put("fintech_use_num", map.get("fintech_use_num")); // 입금계좌(= 환불계좌) 핀테크이용번호
		...
		joReq.put("transfer_purpose", "TR"); // 이체용도(송금(TR), 결제(ST), 충전(RC) 등) : 현재는 돈보내야하니까 송금
		...
		// >>>>>>> depositResult 
		// : {api_tran_id=5aa92b1b-24f8-45a7-a142-9d7965f75660,
		// rsp_code=A0009, rsp_message=API 세부업무 처리실패(리스트 건별 처리결과 확인 요망), 
		// api_tran_dtm=20240129114734411, wd_bank_code_std=002, wd_bank_code_sub=0000000, wd_bank_name=KDB산업은행,
		// wd_account_num_masked=, wd_print_content=eri1112_환불, wd_account_holder_name=, res_cnt=1,
		// res_list=[{tran_no=1, bank_tran_id=M202113854UIXN4JKPPO, bank_tran_date=20240129, bank_code_tran=098, bank_rsp_code=818, 
		// bank_rsp_message=시뮬레이터　응답전문　조회에　실패하였습니다, fintech_use_num=, account_alias=, bank_code_std=002, 
		// bank_code_sub=0000000, bank_name=KDB산업은행, account_num_masked=, print_content=아이티윌＿환불, 
		// account_holder_name=, tran_amt=5014, cms_num=, savings_bank_name=, withdraw_bank_tran_id=}]}
		// => 시뮬레이터　응답전문　조회에　실패하였습니다 ===> 테스트 데이터 등록하러 갑시다
		// => 마이페이지 - 응답정보관리 - 입금이체 - 데이터추가
		// => jo.put("cntr_account_num", "23062003999") <- 이 계좌의 접근 권한이 강사님 토큰 밖에 없어서 admin의 토큰을 DB에서 강제로 덮어씀
	...
	public Map<String, Object> requestDeposit2(Map<String, String> map) {
		...
		joReq.put("bank_tran_id", bankValueGenerator.getBankTranId()); // 거래고유번호(자동생성)
		joReq.put("fintech_use_num", map.get("recv_client_fintech_use_num")); // 입금계좌 핀테크이용번호(상대방 핀테크 번호)
