...
public class SendMailClient {
	...
	public void sendMail(String email, String subject, String content) {
		...
		try {
			...
			// c.f. 대표 프로토콜: http, https, ftp, sftp, smtp, pop3, imap 등

			props.put("mail.smtp.host", "smtp.gmail.com"); // 구글(Gmail)의 SMTP 서버 주소(실제 존재)
			props.put("mail.smtp.port", "587"); // Gmail 서버 TLS 프토로콜 SMTP 포트 번호
			// => 위의 서버로 보내기 위한 보안과 관련된 프로토콜 포트번호
			props.put("mail.smtp.auth", "true"); // SMTP 서버 접근 시 인증 여부 설정(로그인 필요 여부 설정) 


			// 메일 서버 인증 관련 추가 정보 설정(설정 내용에 따라 위의 포트 번호 바뀜 -> 현재는 표준 설정)
			props.put("mail.smtp.starttls.enable", "true"); // TLS 프로토콜(보안 관련 프로토콜) 사용 여부 설정 -> true시 사용
			// => 로그인 해야하는 서버이므로 true 주고 false 줄 경우 로그인 필요없는 경우임

			props.put("mail.smtp.ssl.protocols", "TLSv1.2"); // 위의 TLS 인증 프로토콜의 버전 설정
			props.put("mail.smtp.ssl.trust", "smtp.gmail.com"); // SSL 인증에 사용할 신뢰 가능한 서버 주소(위의 서버와 동일)
			...
			Address senderAddress = new InternetAddress("ier1994@hanmail.net", "테스트");
...
//		message.setHeader("content-type", "text/html; charset=UTF-8");
			...
			message.setSentDate(new Date()); // 객체 생성 시 현재 시스템의 시간, 날짜 사용함
			...
			// => 1~7 전부 try/catch 블럭처리(alt + shift + Z)
