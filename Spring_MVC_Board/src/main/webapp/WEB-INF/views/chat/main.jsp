...
// 		console.log("ws 객체 : " + ws);
		
		// WebSocket 객체의 onxxx 이벤트에 각각의 함수 연결
		// => 주의! onxxx 이벤트에 전달하는 함수는 반드시 함수명만 기술
		ws.onopen = onOpen; // 자바스크립트에서 괄호 명시하면 함수 바로 호출이므로 콜백함수에서 호출되는 함수는 ()없이 함수명만 기술
...
// 		ws.send(msg);
		...
		ws.send(getJsonString("TALK", nickname, msg)); // 대화이므로 "TALK"로 지정
...
// 		appendMessage(">>>>>>>>>>>>>>>>>>>>>>>> onError <<<<<<<<<<<");
		console.log(event); // 콘솔에 event 객체 넘어옴
		appendMessage(">>> 시스템 오류 발생 <<<");
		appendMessage(">>> 잠시 후 다시 접속해 주세요 <<<");
